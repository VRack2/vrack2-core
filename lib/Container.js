...
/***** ********      DEVICE ERROR      ********************/
ErrorManager_1.default.registerMany('Container', [
    {
    ...
    },
    {
        ...
        rules: { device: Rule_1.default.string().description('Device ID') }
    },
    {
...
ErrorManager_1.default.registerMany('Container', [
    {
        ...
        rules: { port: Rule_1.default.string().description('Incorrect port name') }
    },
    {
    ...
    },
    {
        ...
        rules: { port: Rule_1.default.string().description('Port name') }
    },
]);
/***** ********      CONNECTION ERROR      ********************/
ErrorManager_1.default.registerMany('Container', [
...
class Container extends events_1.default {
    ...
    constructor(id, bootstrap) {
        ...
        */
        ...
        this.on('device.alert', (e) => this.noteDeviceMessage(e.device, 'alert', e.data, e.trace));
        this.on('device.error', (e) => this.noteDeviceMessage(e.device, 'error', e.data, e.trace));
        // device.terminate is a critical error message — it lands in lastError too.
        this.on('device.terminate', (e) => this.noteDeviceMessage(e.device, 'error', e.data, e.trace));
    }
    /**
    ...
    */
     ...
     */
    noteDeviceMessage(id, channel, data, trace) {
        const st = this.deviceStatus[id];
        if (!st)
            return;
        // normalize Error traces (device.terminate passes a raw Error object)
        if (channel === 'error' && trace instanceof Error)
