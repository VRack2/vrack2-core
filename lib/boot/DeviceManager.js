...
ErrorManager_1.default.register('DeviceManager', 'DM_DEVICE_NOT_FOUND', 'Device not found', {
    device: Rule_1.default.string().required().example('Master').description('Device name')
});
ErrorManager_1.default.register('DeviceManager', 'DM_LIST_NOT_FOUND', 'list.json not found', {
    vendor: Rule_1.default.string().required().example('vrack').description('Device group name')
});
ErrorManager_1.default.register('DeviceManager', 'DM_LIST_INCORRECT', 'list.json is not correct', {
    vendor: Rule_1.default.string().required().example('vrack').description('Device group name')
});
ErrorManager_1.default.register('DeviceManager', 'DM_VENDOR_NOT_FOUND', 'Vendor not found', {
    vendor: Rule_1.default.string().required().example('vrack').description('Vendor name')
});
ErrorManager_1.default.register('DeviceManager', 'DM_GET_INFO_EXCEPTION', 'Error retrieving device data, see error in attachment', {});
/**
...
class DeviceManager extends BootClass_1.default {
    ...
    checkOptions() {
        return {
            dir: Rule_1.default.string().required().default('./devices'),
            systemDir: Rule_1.default.string().required().default(ImportManager_1.default.systemPath()),
        };
    }
    process() {
