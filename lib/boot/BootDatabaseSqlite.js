...
const BootDatabase_1 = __importDefault(require("./BootDatabase"));
 ...
 */
class BootDatabaseSqlite extends BootDatabase_1.default {
    constructor() {
        super(...arguments);
    ...
    checkOptions() {
        return {
            file: Rule_1.default.string().required().description('Path to the SQLite database file ("":memory:" for in-memory — or use BootDatabaseMemory)'),
            wal: Rule_1.default.boolean().required().default(true).description('Enable WAL journal mode (no effect on ":memory:" or read-only databases)'),
            readOnly: Rule_1.default.boolean().required().default(false).description('Open the database in read-only mode (no writes, no WAL)')
        };
    }
    /**
    ...
    async connect() {
        let driver;
        try {
            driver = await Promise.resolve().then(() => __importStar(require('node:sqlite')));
        }
        catch (e) {
            throw Object.assign(new Error(`The "node:sqlite" module is not available in this Node.js runtime — it requires Node.js 22.5 or newer: ${e?.message}`), { code: String(e?.code ?? 'ERR_MODULE_NOT_FOUND') });
        }
    ...
    async _query(sql, params, conn) {
        const stmt = this._prepare(sql);
        return stmt.all(...(params ?? []));
    }
    async _execute(sql, params, conn) {
        const stmt = this._prepare(sql);
        const res = stmt.run(...(params ?? []));
        const result = { affectedRows: Number(res.changes) };
        ...
        if (res.lastInsertRowid > 0 && /^\s*insert\s/i.test(sql))
            result.insertId = Number(res.lastInsertRowid);
        return result;
    }
    /**
    ...
    async acquire() {
        if (this._txOpen)
            throw ErrorManager_1.default.make('DBS_BUSY', { boot: this.id });
        this._txOpen = true;
        return this._handle;
    }
    /**
...
exports.default = BootDatabaseSqlite;
