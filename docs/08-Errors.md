...
| `vCode` | Уникальный случайный код (для машинной обработки). |
| `vShort` | Читаемый код (например, `CTR_DEVICE_NF`). |
...
| Метод | Значение |
|---|---|
| `register(name, code, short, description, rules?)` | Регистрация ошибки (до создания). Дубликат с другим паром code/short — `EM_CODE_EXISTS`. |
...
| `isCode(error, code)` | `true`, если `vShort` или `vCode` совпадают. |
...
| Код | Описание |
|---|---|
| `EM_CODE_EXISTS` | Дублирование кода/short при регистрации. |
