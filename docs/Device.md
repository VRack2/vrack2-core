...
#### preProcess()
Вызывается на этапе инициализации устройства (до создания соединений). Тут еще есть возможность поменять количество портов, создать обработчики для входящих портов, или например обработчики для экшенов. На этом этапе только создается класс и назначаются основные свойства. Инициализация `shares` присваиванием не вызывает авто-рендер (watcher подключается после `preProcess()`).
...
**Авто-рендер.** После `preProcess()` Container включает реактивность `shares` (объект опирается на `ReactiveRef`): любое изменение автоматически вызывает `render()`:
- запись в существующее свойство (`this.shares.on = true`)
- добавление нового свойства (`this.shares.x = 1`)
- удаление свойства (`delete this.shares.x`)
- полная замена (`this.shares = {...}`)
- вложенные plain-объекты отслеживаются рекурсивно

Ограничения и контракты:
- Массивы внутри `shares` НЕ отслеживаются — чтобы вызвать рендер, переприсвойте массив целиком
- Записи в `shares` внутри `preProcess()` рендер НЕ вызывают (watcher подключается после `preProcess()`)
- В подклассе можно объявить `shares = {...}` полем класса — это значение становится дефолтным `shares`: Container импортирует его в реактивный ref сразу после `preProcess()` (уточнения и замены в `preProcess()` сохраняются). При ES2022-семантике class fields (нативный Node, современные бандлеры) такое поле затеняет accessor базового класса — Container нормализует это автоматически
- В событии `device.render` поле `trace` — это **снимок** `shares` на момент рендера: глубокая обычная копия без прокси реактивности. Подписчики должны считать его read-only: мутировать `shares` прямо внутри обработчика не нужно (повторный рендер из обработчика не вызывается — защита от реентерабельности). Снимок можно безопасно пересылать через воркеры (`postMessage` / structured clone) — прокси сериализоваться не умеют
- В `shares` держите plain-данные (примитивы, объекты, массивы): `Date`, `Map` и экземпляры классов реактивностью НЕ оборачиваются и передаются как есть
...
Инициализируйте `shares` здесь присваиванием — такие записи еще не вызывают авто-рендер.
...
#### render()
Отправляет данные из `shares` подписчикам (событие `device.render`). Вызывается всегда, даже если данные не изменились.
В событие уходит снимок `shares` — глубокая обычная копия без прокси, поэтому данные можно пересылать дальше через воркеры.
После `preProcess()` вызывается автоматически при любом изменении `shares` (см. раздел `shares`).
Старая схема `this.shares.x = 1; this.render()` продолжает работать: придут два события,
что безвредно, т.к. устройство просто ставится в очередь отправки данных.
