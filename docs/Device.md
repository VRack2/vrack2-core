...
- В подписчиках `device.render` объект `trace` считается read-only: изменение `shares` прямо внутри обработчика не вызовет повторный рендер (защита от реентерабельности)
...
#### render()
Отправляет данные из `shares` подписчикам. Вызывается всегда, даже если данные не изменились.
