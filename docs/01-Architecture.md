...
3. **Создаются устройства — по одному.** Для каждого устройства из `service.json`:
   ...
   - отдаёт устройство `Container` — тот его **регистрирует**: вызывает `preProcess()` (начальные `shares` из поля подкласса подключаются после него), включает авто-рендер `shares`, проверяет, что у каждой объявленной action есть хендлер, регистрирует метрики, создаёт входы/выходы.
