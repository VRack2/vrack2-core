...
- Остановленное устройство не принимает данные: состояние `running` управляет контейнером (`startDevice()` / `stopDevice()`), а порты не-running устройства дропят `push`. Actions на остановленное устройство отклоняются ошибкой `CTR_DEVICE_STOPPED`.
